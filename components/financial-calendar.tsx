...
export default function FinancialCalendar({
  ...
  const isHighSpendingDay = (date: Date) => {
    return highSpendingDays.some((d) => d.toISOString().split("T")[0] === date.toISOString().split("T")[0])
  }

  // Get total spending for selected date
  const getTotalSpending = () => {
    return selectedDateTransactions.filter((t) => t.amount < 0).reduce((sum, t) => sum + Math.abs(t.amount), 0)
  }

  // Get total income for selected date
  const getTotalIncome = () => {
    return selectedDateTransactions.filter((t) => t.amount > 0).reduce((sum, t) => sum + t.amount, 0)
  }
    ...
    <div className="space-y-4">
      ...
      <div className="mt-4 flex items-center justify-between text-sm text-muted-foreground">
        <div className="flex items-center gap-2">
          ...
          <span>
            {selectedDate ? format(selectedDate, "PPP", { locale: language === "vi" ? vi : enUS }) : t.selectDate}
          </span>
        </div>
        <div className="flex items-center gap-2">
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-primary/10 rounded-lg p-3 text-center">
                    ...
                    <p className="text-lg font-bold text-primary">{formatCurrency(getTotalIncome())}</p>
                  </div>
                  <div className="bg-destructive/10 rounded-lg p-3 text-center">
                    <p className="text-sm text-muted-foreground">Expenses</p>
                    <p className="text-lg font-bold text-destructive">{formatCurrency(getTotalSpending())}</p>
                  </div>
                </div>
