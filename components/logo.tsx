  ...
  return (
    ...
    >
      <motion.div className="relative" variants={logoVariants}>
        <Coins className={cn("h-6 w-6", isDark ? "text-primary" : "text-primary")} />
        <motion.div
          className={cn("absolute -right-1 -top-1 h-2 w-2 rounded-full", isDark ? "bg-primary" : "bg-primary")}
          variants={dotVariants}
        />
      </motion.div>
      <motion.span
        className={cn("font-bold text-xl", isDark ? "text-white" : "text-foreground")}
